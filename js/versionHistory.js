    ...
    launcherUpdateContent: [
        ...
        },
        {
            ...
            features: [
                ...
                "- 音乐播放器自定义背景：右下角折页为背景入口，鼠标悬浮折页时当前曲目背景以 18% 淡透明预览，点击折页后背景完全铺满整个播放区（再次点击空白区域或按 Esc 收起）；右上角设置按钮打开播放器设置弹窗，支持为每首曲目单独上传本地图片作为专属背景——背景按曲目独立存储、互不影响，切换歌曲自动展示对应背景（单张背景仅对应一首曲目），弹窗内含 16:9 实时预览、3MB 图片大小限制校验、保存与一键清除功能",
                "- 音乐播放器新增「调整播放器组件显示状态」功能：在播放器设置弹窗中可独立开关唱片封面与歌曲名称/艺人信息的显示（进度条与控件按钮等核心功能区始终保持可见不可隐藏），设置通过 localStorage 持久化，刷新后自动恢复",
                "- 音乐播放器新增「调整播放器组件位置和大小」功能：设置弹窗内提供九宫格位置预设（左上/左中/左下/上方/居中/下方/右上/右中/右下）与70%~130%缩放滑块，实时预览、一键恢复默认，所有布局设置通过 localStorage 持久化",
                ...
                "- 音乐播放器设置交互全面重构（设置弹窗改为侧边栏设置模式）：播放器设置入口由右上角设置按钮改为播放列表标题右侧的齿轮按钮，点击后不再弹出设置弹窗，而是整个播放器进入「播放器设置」模式——左侧边栏平滑切换为设置条目菜单（含「自定义背景图片（按曲目独立）」「调整播放器组件位置和大小」「调整播放器组件显示状态」三个条目，附图标与说明文字），右侧内容区停止显示唱片与歌曲信息、改为展示所选举目对应的功能面板；侧边栏标题同步切换为「播放器设置」并显示副标题，附返回按钮可随时一键返回播放列表，原有设置弹窗中的全部功能均已迁移至新面板并适配新布局样式，所有更改即时生效并通过 localStorage 持久化",
                ...
                "- 音乐播放器新增播放列表管理系统：播放列表头部设置按钮左侧新增「更多」（…）按钮，点击弹出多级下拉菜单，内含「新建播放列表」与「删除播放列表」两个条目；点击「新建播放列表」后立即创建并自动切换到一个空的全新播放列表（自动去重命名，如「新建播放列表 2」），侧边栏即时刷新为空列表状态",
                ...
                "- 音乐播放器新增「播放器控制条设置」条目：播放器设置侧边栏在「调整播放器组件显示状态」下方新增第四个设置条目，点击后右侧内容区显示对应功能面板，内含「离开音乐播放器后始终显示底部控制条」开关（默认开启）——开启时正在播放音乐的前提下切换到页面其他条目仍会显示底部控制条，关闭后离开播放器即不再显示，更改即时生效并通过 localStorage 持久化，刷新后自动恢复",
                ...
                "- 音乐播放器设置弹窗尺寸加大并改为两列排版：弹窗宽度由460px增至720px，内部以左右两列 flex 布局分别展示背景设置与布局/显隐设置（窄屏 ≤780px 自动降为单列），减少滚动条使用",
                "- 音乐播放器设置弹窗滚动条样式自定义：滚动条改为6px宽细条、紫色主题半透明滑块（rgba(118,75,162,0.35)），hover加深，同时适配 Firefox（scrollbar-width: thin + scrollbar-color）",
