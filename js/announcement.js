...
const announcementData = {
    ...
    normalAnnouncements: [
        {
            id: "normalA-20260919",
            title: "关于经验值补给卡导致页面卡死问题的修复与补偿说明",
            date: "2026-09-19",
            tag: "notice",
            tagText: "通知",
            author: "GPY Games Studio - PREAlmax",
            ...
            content: [
                "大家好，这里是 PREAlmax。",
                "",
                "今天我们要向大家说明一个经验值系统的重要问题：部分用户反馈在超过 60 级后使用经验值补给卡，会导致页面系统直接卡死、失去响应；强关页面重新进入后，已使用的补给卡会被算作直接消耗，但账户经验值却没有正确提高。目前该问题已在 RC 3.0.2.6 (c3) 版本中修复，并已向受影响的用户发放补偿，具体说明如下：",
                "",
                "[color:#ff6b6b]【问题说明】[/color]",
                "当角色等级超过 60 级（即已达到 60 级满级状态）时，使用任意一张经验值补给卡（经验值补给卡 Ⅰ / Ⅱ / Ⅲ 等）后，页面系统会立即卡死、完全失去响应；此时强关页面重新进入，会发现刚刚使用的补给卡已被算作直接消耗并从仓库中扣除，但账户经验值却没有得到任何提升——即出现「吞物品、不加经验」的情况。",
                "",
                "[color:#ffd93d]【问题原因】[/color]",
                "该问题的根因是经验值系统的升级循环缺少等级上限保护：当角色达到 60 级满级状态时，升级所需经验值会被置为 0，而补给卡等外部经验发放入口所使用的升级循环没有对等级上限进行检查，导致循环条件永远成立并无限执行，页面因此卡死；而经验值数据的保存逻辑位于循环之后，也就永远不会被执行到，最终造成「道具已消耗、经验未入账」的结果。",
                "经排查，该 bug 可能在经验值系统更新后就已出现，只是需要「等级达到 60 级满级」这一前置条件才会触发，直至近期阶段突破功能上线后才被注意到。",
                "",
                "[color:#4ecdc4]【处理措施】[/color]",
                "我们已在 RC 3.0.2.6 (c3) 版本中紧急修复该问题：升级循环现已加入等级上限保护，满级状态下使用补给卡不再卡死，超出当前等级的溢出经验值将完整保留在账户内；同时经验值公式已与主系统完全同步，阶段突破模式下 60 级以上也可正常使用补给卡并按新公式升级。",
                "",
                "[color:#d45d79]【补偿说明】[/color]",
                "对于此前因该问题而被消耗但未入账的补给卡，我们深表歉意，并已向所有符合条件的用户发放补偿邮件，补偿内容为：经验值补给卡 Ⅰ ×1、经验值补给卡 Ⅲ ×1 和 自选物品兑换卡 ×1。",
                "补偿邮件发放对象为 2026-09-19 18:40:00 (UTC+8) 之前完成注册的账户；领取有效期截至 2026-09-26 23:59:59 (UTC+8)，逾期未领取将无法补发，请您及时前往邮箱领取。",
                "",
                "[color:#d45d79]【写在最后】[/color]",
                "对于该问题给您带来的不便与损失，我们再次诚挚致歉。",
                "您的每一条反馈都是我们发现问题、改进产品的重要途径，衷心感谢每一位用户的支持与理解！",
    ...
    devLogs: [
        {
            ...
            images: [],
            content: [
                "今天我们发布了 RC 3.0.3.0 (c3) 重大版本更新！本次更新以全新「赛季通行证系统」为核心——独立于账户等级的全新等级体系正式上线，数据按账户隔离存储于 localStorage，与现有成长等级完全互不影响；第一赛季「初始化」(Season 1 - Initialize) 定于 2026-10-01 08:00 至 2026-12-31 17:00 (UTC+8) 开启，等级上限 120 级，每级需 10 经验值，购买一级需 300 PRE Coin；通行证等级奖励体系提供 120 级动态奖励表（每级免费档位 1 件物品 + 付费档位 3 件额外物品，付费通行证售价 6480 PRE Coin），满级后新增 EX 溢出等级无上限续升；通行证三层任务系统（日常/周常/赛季）自动接入签到、PRE Coin 变化、仓库/商店打开、经验卡使用、登录等 6 个行为入口；通行证 UI 全屏弹窗采用 GSAP 入场动画 + Three.js 3D 粒子背景 + 双 Tab 切换布局；同时上线通行证组合包（12900 PRE Coin，含 10 级等级、补给卡与限定纪念徽章）、物品详情弹窗、每日签到通行证经验奖励、活动中心赛季活动分类等全新功能，并对通行证窗口滑动卡顿、仓库物品图标与风味文本、经验加成叠加上限等进行了全面优化与修复！",
                "[color:#d45d79]【新增功能】[/color]",
                "• 赛季通行证系统正式上线：独立于账户等级的全新等级体系，数据按账户隔离存储于 localStorage（pass_<username>），与现有账户等级完全互不影响；新增 js/pass.js 承载全部通行证数据与逻辑",
                "• 第一赛季「初始化」(Season 1 - Initialize) 开启：赛季时间 2026-10-01 08:00 UTC+8 至 2026-12-31 17:00 UTC+8，等级上限 120 级，每级需 10 经验值，购买一级需 300 PRE Coin",
                "• 通行证等级奖励体系：120 级动态奖励表，每级免费档位 1 件物品 + 付费档位 3 件额外物品（付费通行证售价 6480 PRE Coin）；按 6 档递增物品阶级，低等级多给数量、高等级重质不重量；满级后新增 EX 溢出等级，免费每溢出 10 经验领 50 PRE Coin、付费额外 100 PRE Coin",
                "• 通行证三层任务系统：日常任务（每日刷新，最多 5 个，每个 +2 经验）、周常任务（每周一刷新，最多 5 个，每个 +4 经验）、赛季任务（赛季内固定 10 个不刷新，+5~8 经验）；任务进度自动接入签到、PRE Coin 变化、仓库/商店打开、经验卡使用、登录等 6 个行为入口",
                "• 通行证 UI 全屏弹窗：GSAP 入场动画（分级淡入+弹性缩放）+ Three.js 3D 粒子背景（40 粒彩色粒子缓慢旋转）+ Tailwind 渐变布局，支持赛季通行证/通行证任务双 Tab 切换",
                "• 购买等级弹窗：Hero 区单按钮触发，弹窗内 +- 号步进输入 + 实时 PRE Coin 计算 + 余额不足警告，确认后弹出二次确认弹窗才执行购买",
                "• 购买付费通行证弹窗：展示 6480 PRE Coin 价格及本期全部可获取付费物品清单（按物品汇总数量）",
                "• 通行证购买方式选择弹窗：点击「解锁付费通行证」按钮后先弹出加宽选择弹窗「是否要进行购买？我们为您提供了以下两种组合方式」，内含「第一赛季「初始化」通行证本体」与「第一赛季「初始化」通行证组合包」两个块状按钮（带图标、说明与价格），分别进入本体购买弹窗与组合包购买弹窗；已拥有本体时本体卡片置灰标注「已拥有」不可重复购买",
                "• 通行证组合包上线：定价 12900 PRE Coin，包含通行证本体全部付费内容，购买后立即获得 10 级通行证等级、经验值补给卡Ⅳ×2、PRE Coin 补给包Ⅳ×1 与全新「第一赛季纪念徽章」（可展示在用户名片内）；组合包弹窗在保留本体全部付费福利清单的基础上新增立即获得内容展示区与组合包专属说明；未购买本体时直接购买组合包享 80% 优惠价 10368 PRE Coin（展示划线原价与「限时 8 折」标签），已购买本体则按原价 12900 PRE Coin 购买且不再享受优惠",
                "• 仓库与个人荣勋新增「第一赛季纪念徽章」：限定徽章（火箭图标、通行证主题粉配色，含专属介绍与风味文本），购买通行证组合包后立即发放至仓库徽章分类下；用户名片「个人荣勋」自动同步识别（解锁条件显示为购买组合包，支持解锁时间记录与详情查看）",
                "• 通行证本体定价调整：付费通行证售价由 5000 PRE Coin 上调至 6480 PRE Coin，购买弹窗、Hero 区购买提示与余额校验同步更新",
                "• 活动公告新增「第一赛季「初始化」即将上线」预告公告：归入「即将到来」分类（随赛季时间自动流转至当前活动/已结束），公告内容含上线时间、赛季亮点（120 级奖励长线/三层任务系统/EX 溢出奖励/通行证组合包与纪念徽章）、购买方式与赛季前锁定提示；公告详情底部自动关联「跳转至相应活动」按钮直达活动中心赛季活动条目",
                "• 活动中心新增「赛季活动」分类与「第一赛季「初始化」」活动条目：活动时间与赛季时间一致（2026-10-01 08:00 至 2026-12-31 17:00 UTC+8，由 SEASON_PASS_EVENT 统一配置并与 pass.js 赛季时间对齐）；赛季未开始时活动状态显示「未开始」、「立即参与」按钮置灰不可点击（点击兜底提示活动尚未开始）；赛季开启后点击「立即参与」直接打开赛季通行证界面，活动结束后自动归入已结束分类",
                "• 通行证新增「全部领取」按钮：与滚动提示、Dev 按钮一同置于「赛季通行证/通行证任务」Tab 栏右侧工具组，一键领取当前全部可领取的等级奖励（免费+付费），按钮实时显示可领取等级数，无可领奖励时自动置灰，领取结果以提示横条汇总展示",
                "• 通行证赛季未开始锁定机制：赛季开始时间前，头部标签显示灰色「未开始」而非绿色「进行中」，购买等级与解锁付费通行证按钮锁定禁用，通行证任务中的日常/周常/赛季任务全部锁定（任务进度不累积、无法领取经验），等级奖励领取与 EX 溢出奖励同步锁定，防止用户在错误的时间段解锁通行证等级",
                "• 邮件领取记录新增展开/收起按钮：每封邮件标题右侧新增可点击的展开/收起按钮（带方向旋转动画），所有邮件默认为收起状态，点击后展开显示领取内容（发件人、领取时间、奖励明细）以及该邮件的完整文本正文；旧记录（无 content 字段）自动通过新增的 getHistoryItemContent 函数从当前邮箱或邮件模板中回溯获取正文内容；移动端、暗色模式、透明模式全部适配",
                "• 物品详情弹窗全新上线：仓库中点击任意物品卡片、邮件附件中的仓库型物品、邮件领取记录中的仓库型奖励均可弹出统一的物品详情弹窗——弹窗宽度 780px，左侧约 1/3 为物品图标区，右侧自上而下依次为物品名称、分类标签、详细介绍、斜体风味文本展示区，右下角「使用该物品」按钮仅对可使用且当前拥有的物品显示；弹窗 z-index 100001 置于仓库弹窗与邮件弹窗之上，ESC 关闭使用捕获阶段事件阻止冒泡防止下层弹窗同步关闭；预览模式（previewOnly）支持邮件附件与领取记录中物品的只读查看",
                "• 全部 15 个仓库物品新增 flavorText 风味文本字段：每个物品均配有一段斜体风味文本，使物品介绍更丰满更有故事感",
                "• 仓库新增 5 个对启动器现有系统有效果的道具：经验值加成卡 Ⅳ（传说，30 分钟内经验 +50%，新增 premium 类型，可与 I/II/III 中任意两张叠加至 +100%）；PRE Coin 补给包 Ⅰ-Ⅳ（普通/稀有/史诗/传说，分别获得 100/300/600/1500 PRE Coin，直接对账户余额生效）",
                "• 每日签到新增赛季通行证经验值固定奖励：赛季真实进行期间每日签到固定获得 +3 通行证经验值；该奖励为赛季固定值，不随连签天数、断签、阶段突破或经验倍率活动变化，始终为 +3；通行证满级后自动计入 EX 溢出经验；签到结果弹窗新增粉紫皇冠图标区块展示本次通行证经验奖励",
                "• 每日签到「每日奖励」条目改为可点击按钮：参照「本月奖励一览」「下一里程碑」样式，点击后弹出「每日签到·今日可领取内容」详情弹窗，卡片式分区展示成长经验值（随连签/阶段突破变化的说明）、PRE 硬币（随连签提升）、今日签到道具（真实图标、名称、数量标签，注明 14 天限时）与通行证经验值（赛季中显示 +3 并标注「每日固定」，非赛季显示赛季未开始/已结束的置灰说明）；弹窗顶部区分「今日已签到/尚未签到」状态，底部提示各奖励的变化规则",
                "• 通行证等级卡片物品详情弹窗：点击通行证等级卡片内免费/付费档位的物品槽即可弹出与仓库完全一致的物品详情弹窗（复用 showWarehouseItemDetail 预览模式），弹窗展示该等级的奖励数量、物品分类标签、详细介绍与斜体风味文本；物品详情弹窗 z-index 动态提升至 1000000 确保浮于通行证全屏弹窗之上；已领取与未解锁状态的物品槽同样支持点击预览",
                "• 通行证头部赛季剩余天数标签：起始/结束日期标签右侧新增琥珀色高亮标签（沙漏图标），实时显示「本赛季剩余 X 天」；赛季未开始时显示「赛季未开始」、赛季已结束时显示「已结束」，与赛季时间监测联动自动切换",
                "• 通行证任务栏刷新倒计时标签：日常/周常/赛季子标签栏右侧新增琥珀色刷新倒计时标签（时钟图标），按当前子页动态显示——日常显示「距离下一次日常刷新还剩 X 天」（按 UTC 日期刷新键对齐）、周常显示「距离下一次周常刷新还剩 X 天」（以周一为起点计算）、赛季与赛季剩余天数保持一致；移动端标签独占一行居中显示",
                "• 通行证满级后 EX 等级进度持续显示：等级达到 120 级满级后，每继续获得 10 点溢出经验，顶部等级数字即递增 1 级（120 → 121 → 122……），自然升级无等级上限；经验条在满级后不再恒满，而是按当前 EX 等级内进度重新蓄满，经验文本显示「EX x / 10」，EX 溢出横条文案同步改为「EX 溢出：N 级（每 10 经验 +1 级，无上限）」；右上角「等级上限 120」统计在进入 EX 阶段后变为「∞ · EX 无上限」，每级 EX 的 PRE Coin 领取规则（免费 50、付费额外 100）保持不变",
                "• 通行证右上角新增 PRE Coin 余额显示：赛季通行证窗口 Hero 区统计栏首位新增金币图标 + 当前 PRE Coin 余额（金色数字），领取等级奖励、领取 EX 溢出奖励等操作后随界面刷新自动同步，无需打开商店即可查看余额",
                "[color:#4ecdc4]【优化改进】[/color]",
                "• 通行证弹窗布局放宽：容器取消 1400px 最大宽度限制，弹窗内全部 UI 组件向左右两侧扩展铺满浏览器窗口，仅与窗口边缘保留少量间距",
                "• 通行证等级卡片双档位重构：每级卡片内的三件奖励物品分为「免费」「付费」两个独立挡位条目（各带标题标签与实时状态：可领取/已领取/需解锁付费通行证），层级划分更清晰",
                "• 通行证等级卡片调矮加宽：卡片内边距与间距全面收紧使高度明显降低，宽度由 280px 加长至 350px，奖励区不再出现右侧竖向滚动条，正常横向滚动不受影响",
                "• 通行证工具组移至 Tab 栏：「左右滑动查看更多等级奖励」提示、「全部领取」及全部 Dev 功能按钮由赛季通行证内容区移至「赛季通行证/通行证任务」Tab 栏右侧工具组，内容区不再因工具条占用高度而出现右侧竖向滚动条",
                "• 通行证领取奖励烟花粒子效果移除：因运行卡顿删除领取时的彩纸飞散动画（含 Three.js 逐帧物理更新），仅保留轻量的按钮缩放反馈",
                "• 通行证赛季时间显示优化：起始/结束时间由 UTC 改为 UTC+8 显示（起始 2026-09-21 19:00、结束 2026-12-21 17:00）",
                "• 通行证奖励表限量重分配：经验值加成卡Ⅳ全赛季≤8张、PRE Coin 补给包Ⅳ≤5份、PRE Coin 补给包Ⅲ≤10份、经验值补给卡Ⅳ≤8张、经验值补给卡Ⅲ≤12张，其余物品重新匀配（整十级免费档位固定给十连抽券）；每级免费档位新增 PRE 硬币奖励（每级 40 枚、逢 6 的倍数等级 50 枚），全赛季合计 5000 PRE 硬币",
                "• 邮件领取记录默认折叠优化：所有历史邮件默认为收起状态，避免长正文撑开列表导致界面冗长，按需点击展开按钮查看完整内容",
                "• 物品详情弹窗仅保留分类标签（消耗品/材料/徽章）；稀有度配置保留用于排序与边框配色不受影响",
                "• useWarehouseItem 道具使用逻辑扩展：新增 _precoinGain 字段支持，PRE Coin 补给包与综合补给包均接入现有 addPreCoin 函数（silent=true 避免与统一提示横条重复）；Toast 提示覆盖幸运币、经验值补给、PRE Coin 补给、综合补给四种场景",
                "• 自选物品兑换卡新增经验值加成卡 Ⅳ 与 PRE Coin 补给包系列共 5 个兑换选项：修复 shop.js 中 SHOP_ITEM_PRICES 缺失条目导致兑换卡无法识别新物品的问题",
                "• 经验加成叠加上限由 3 张提升至 4 张：activateWarehouseExpBuff 上限判断改为 >= 4，支持 I+II+III+Ⅳ 四张同时激活叠加至 +100%，四张卡片描述与仓库统计标签同步更新",
                "• 全部 22 个仓库物品图标升级：在 Font Awesome 6.0.0 Free 范围内重新选取各有辨识度的图标家族（经验加成卡用速度表/上升图表/火箭/火焰、经验补给卡用星/原子/星号/太阳、PRE Coin 补给包用硬币堆/存钱罐/钱包/礼盒），避免使用 FA 6.1+ 才有的 Pro-only 图标；分类标签从 fa-medal 改为 fa-id-badge 避免与先驱者勋章重复",
                "• 全部 22 个仓库物品风味文本换行化：统一为「场景描述 \\n —— 感悟点评」双行格式，.wh-detail-flavor-text 新增 white-space: pre-wrap 确保 \\n 正确换行",
                "• 通行证等级卡片样式提亮：已领取等级卡片由半透明灰改为绿色渐变高亮底 + 绿边框 + 微发光（opacity 恢复为 1），与未解锁卡片（暗灰 opacity 0.5）形成鲜明对比，便于快速辨认领取状态；卡片内物品名称亮度由 0.85 提至 0.96 并加文字投影，数量文字由 0.7 提至 0.9，已领取的物品槽带淡绿底色",
                "• 每日奖励预览文本补充通行证经验：赛季进行期间「每日奖励」条目预览文本追加「+ 3 通行证经验」，让玩家在签到前即可看到通行证经验奖励",
                "• 每日签到奖励详情弹窗通行证经验值条目新增赛季限时时间说明：条目描述下方追加带时钟图标的「赛季限时可领取时间 2026-10-01 08:00 (UTC+8) 至 2026-12-31 17:00 (UTC+8)」，时间取自 PASS_CONFIG 自动格式化为 UTC+8，赛季开启与未开启态均显示",
                "• EX 溢出奖励改为「一键领取」：满级后 EX 溢出横条中的领取按钮更名为「一键领取」，点击后一次性领取当前全部可领的溢出等级奖励（免费档位每级 +50 PRE Coin、付费档位每级额外 +100 PRE Coin 按等级数叠加发放），无需再逐级反复点击",
                "• EX 溢出奖励领取成功改用奖励弹窗展示：一键领取成功后弹出「领取成功」弹窗（无限符号青绿色徽章），副标题显示本次领取的 EX 等级范围（如 Lv.121 ~ Lv.9879 共 N 级），弹窗内展示 PRE Coin 合计数额及免费/付费档位发放明细说明，替代原先的提示横条；无可领取奖励、赛季未开始等异常场景仍保留提示横条",
                "• 优化通行证窗口动画与滑动等级卡时的卡顿问题：移除通行证全屏弹窗的 backdrop-filter 全屏背景模糊（背景本身接近不透明，视觉无差异），滑动时不再走逐帧全屏模糊合成；移除每个锁定物品槽位的 backdrop-filter，改用更深半透明底色",
                "• 可领取卡片光晕呼吸动画由 box-shadow 逐帧重绘改为伪元素 opacity 合成器动画（高等级下上百张可领取卡同时跑 box-shadow 动画是滑动卡顿的最大来源），视觉效果不变；新增滚动检测，滑动等级卡期间自动暂停光晕/脉冲等无限动画，停止滚动约 0.18 秒后恢复",
                "• 等级奖励卡新增 content-visibility: auto 离屏裁剪，120 张等级卡仅渲染可视区域附近的卡片，离屏卡片完全跳过渲染与动画；背景光斑大模糊元素提升为独立合成层只栅格化一次；奖励区滚动去掉 smooth 平滑模式（JS 滚轮逐次触发平滑动画互相打断导致卡顿感），吸附模式由 mandatory 调整为 proximity 减少自由滑动时的强制吸附回流；卡片 hover 过渡从 transition: all 收窄为指定属性",
                "• 通行证满级后购买等级入口变更：等级达到 120 级（含 EX 溢出阶段）后，「购买等级」按钮永久替换为不可点击的灰色「已满级」按钮（锁形图标），未到满级且赛季未开始/已结束时仍保持禁用态，赛季进行中恢复为可点击的购买入口",
                "[color:#ff6b6b]【修复问题】[/color]",
                "• PRE Coin 补给包 Ⅲ/Ⅳ 图标修复：原使用 fa-sack-dollar/fa-treasure-chest（FA 6.1+ 才引入）导致浏览器渲染为空，替换为 FA 6.0.0 支持的 fa-wallet/fa-box-open",
                "• 自选物品兑换卡弹窗尺寸固定：改为 flex 布局 + 内容区独立滚动（min-height:0 + overflow-y:auto），无论物品数量多少都不拉伸窗口",
                "• 通行证弹窗性能优化：粒子数从 120 降至 40，渲染频率每 3 帧一次（约 20fps），关闭弹窗时暂停 Three.js 渲染循环释放 GPU；移除 Lenis 平滑滚动改用原生滚动避免卡顿",
                "• 通行证弹窗层级修复：粒子 canvas 与 glow 背景 z-index 降至 0，容器升至 2 确保不遮挡按钮与卡片；提示横条(toast) z-index 强制提升至 1000001 高于弹窗",
                "• 通行证奖励区改为横向滚动：flex 布局 + scroll-snap 吸附，卡片宽度 280px、物品名不截断；鼠标滚轮自动转为横向滚动",
                "• 通行证弹窗头部新增赛季时间监测：胶囊式展示起始与结束时间（UTC），响应式适配",
                "• 每日签到结果弹窗通行证经验区块图标修复：使用 Font Awesome 6.0.0 支持的 fa-thumbtack 替代不兼容的 fa-thumb-tack，避免在签到结果弹窗与每日奖励详情弹窗中渲染为空图标",
                "• 修复通行证中首次点击 PRE Coin 查看介绍弹窗时样式未生效的问题：PRE Coin 介绍弹窗依赖仓库样式表（.wh-precoin-* 系列），此前该样式仅在仓库弹窗、物品详情弹窗等创建时注入，若用户未打开过仓库且首次直接从通行证点击 PRE 硬币槽位，样式表尚不存在导致弹窗以裸文本形式错乱显示；现已在 showPrecoinDescription 中补充与其他弹窗一致的样式注入保障，首次打开即可正确渲染弹窗布局（图标面板、余额、介绍分区、感悟文案等）",
                "• 修复购买通行证本体/组合包/等级花费的 PRE Coin 未计入商店「累计消费」的问题：商店累计消费原本只统计商店内商品购买，通行证相关消费虽已正确扣减 PRE Coin 余额却未累加；现新增 shopAddExternalSpend 外部消费记账接口，购买付费通行证本体（6480 PRE Coin）、通行证组合包、购买通行证等级（每级 300 PRE Coin）成功扣费后同步计入商店累计消费统计（不增加商品购买件数），与 PRE Coin 收支历史中的消费记录保持一致",
                "• 修复仓库中经验值补给卡 Ⅰ/Ⅱ（及所有持有数量大于 1 的可批量使用道具）点击使用无反应的问题：批量使用弹窗模板内标题与描述元素使用的是 class（.wh-batch-name/.wh-batch-desc），但填充代码却按 id（#whBatchName/#whBatchDesc）查询，查询结果为 null 导致在弹窗显示与确认按钮绑定之前抛出类型错误、函数中断，表现为点击使用后无任何反应；已将选择器修正为与模板一致的 class 选择器，批量使用弹窗可正常弹出并完成多次使用",
                "[color:#d45d79]【感谢支持】[/color]",
                "感谢您对 PRE Launcher 的持续关注和支持！本次版本更新为启动器带来了全新「赛季通行证系统」——从独立等级体系、120 级动态奖励表、三层任务系统到通行证组合包与限定纪念徽章，致力于为您打造更长线、更有目标的赛季成长体验；同时新增的物品详情弹窗、每日签到通行证经验奖励、活动中心赛季活动分类等功能，以及针对通行证滑动卡顿的全面性能优化、仓库物品图标与风味文本升级、经验加成叠加上限提升等改进，持续丰富着启动器的日常使用体验。",
                "对于本次更新中修复的通行证消费记账缺失、经验值补给卡批量使用无反应等问题给您带来的不便，我们诚挚致歉；如果您在使用过程中发现任何问题，或有任何想法和建议，欢迎通过 Github 仓库提交 Issue 与我们进行沟通。",
                "[color:black]© 2014-2026 PREAlmax. All rights reserved.[/color]"
            ]
        },
        {
            id: "devlog-20260919",
            title: "RC 3.0.2.6 开发日志",
            date: "2026-09-19",
            tag: "update",
            tagText: "公告",
            author: "GPY Games Studio - PREAlmax",
            ...
            images: [],
            content: [
                "今天我们发布了 RC 3.0.2.6 (c3) 重要版本更新！本次更新以全新「阶段突破」模式为核心——当成长等级达到 60 级满级状态后，可开启阶段突破将等级上限提升至全新的 120 级最高等级：60 级以上启用全新分档经验值公式，成长等级卡片显示淡红色「阶段突破」条目与标签，满级后自动弹出确认弹窗（双重红色确认防止误开）；阶段突破模式下每日签到基础经验值与 PRE 硬币大幅提升，签到里程碑弹窗新增「标准阶段 / 阶段突破」双阶段切换与全新独立里程碑奖励；同时新增百宝箱「木鱼」攒功德功能、每日签到结果弹窗展示与音乐播放器配置导入导出，统一开发者弹窗样式动效，并修复了超过 60 级使用经验值补给卡导致页面卡死、补给卡被吞的关键问题！",
                "[color:#d45d79]【新增功能】[/color]",
                "• 全新「阶段突破」模式：当成长等级达到 60 级满级状态后，成长等级卡片底部将出现淡红色渐变的「阶段突破」条目，点击即可了解并开启突破；首次达到满级时会自动弹出确认弹窗（每次满级周期仅自动提示一次，之后可随时点击条目手动打开）。弹窗完整说明突破规则并以红色文本警示「开启阶段突破模式后等级上限将进入120级，无法进行退回或关闭该模式」，点击确定后还会弹出整段红色文本的二级确认弹窗「确定要进入该模式吗？这是您最后一次的选择机会」，再次确定后才正式进入阶段突破模式，双重确认防止误开",
                "• 等级上限突破至 120 级：进入阶段突破模式后，成长等级卡片右上角等级徽章（Lv.X）左侧将显示淡红色渐变「阶段突破」标签；等级上限由 60 级提升至 120 级，同时等级新经验值公式立即启用——Lv.60-89 为 120+(等级+6)×80，Lv.90-109 为 130+(等级+8)×80，Lv.110-120 为 150+(等级+10)×100（60 级以下保持原公式不变），每次升级所需经验值需求量随档位一并增多；用户名片中的等级说明同步显示新的 120 级上限与全部新分档公式",
                "• 阶段突破模式下每日签到奖励大幅提升：由于新经验值公式升级所需经验值数量巨大，为避免升级周期过于漫长，每日签到基础经验值提升为 1-6 天每天 300 EXP、连签 7 天起每天 500 EXP、连签 15 天起每天 750 EXP；每日基础 PRE 硬币同步提升为 1-6 天每天 80、连签 7 天起每天 120、连签 15 天起每天 150（标准模式 20/50/80 不变）；签到卡片提示条、每日奖励预览、用户名片、本月奖励一览等处数值均随模式自动切换显示",
                "• 签到里程碑弹窗新增「标准阶段 / 阶段突破」双阶段切换：弹窗标题「签到里程碑」右侧新增两个胶囊切换按钮——点击「标准阶段」显示正常的 60 级之前签到里程碑内容，点击「阶段突破」切换显示全新的 60-120 级里程碑内容（未开启阶段突破模式时该按钮为禁用灰色态无法点击，悬停提示需先开启突破模式）；两个阶段的里程碑完全独立：阶段突破里程碑不继承标准阶段已完成的里程碑领取记录与累计天数，将再次独立计数、独立领取",
                "• 全新阶段突破里程碑配置：连签 7 天额外 +1000 EXP + 500 PRE 硬币、15 天 +1500 EXP + 500 PRE、30 天 +3000 EXP + 1000 PRE、60 天 +3000 EXP + 1000 PRE、90 天 +3000 EXP + 1500 PRE、180 天 +5000 EXP + 2000 PRE + 自选物品兑换卡×1、365 天 +10000 EXP + 3000 PRE + 自选物品兑换卡×1；奖励发放同样需手动点击里程碑卡片领取，自选物品兑换卡的终身发放记录与标准阶段相互独立",
                "• 百宝箱新增「木鱼」功能卡片：卡片配备木色渐变木鱼图标与「开始攒功德」按钮，点击弹出加宽版弹窗——弹窗上方正中间实时显示功德计数，中部为大尺寸可点击木鱼图标，每点击一次计数 +1 并伴随敲击回弹动画与「功德 +1」上浮文字（支持回车/空格键敲击）；弹窗最下方正中间新增「DDos佛祖」按钮，点击后系统以每秒 20 次的超高频率自动敲击木鱼持续攒功德，按钮同步切换为橙红脉冲态「停止 DDoS」可随时停止；弹窗右下角新增重置按钮，点击后先弹出「是否要重置当前功德计数？」确认框，确定后才清零；关闭弹窗、点击遮罩或按 Esc 时自动停止自动敲击防止后台空转",
                "• 每日签到结果改为完整弹窗展示：点击「立即签到」后不再依靠一闪而过的提示横条，改为弹出加宽版签到结果弹窗——顶部绿色对勾徽章显示「签到成功 · 连续签到 N 天」，中部依次展示经验值卡片（本次所得 EXP、经验倍率与连续签到等加成逐行明细、升级时显示金色「升级啦！」横幅）、PRE 硬币卡片（合计所得与来源明细）与限时道具卡片（道具图标、数量、有效期），存在可领取里程碑时底部同步提示；点击「开心收下」或右上角 ×/遮罩/Esc 关闭，彻底避免提示横条消失过快没看见的问题",
                "• 音乐播放器新增导入及导出功能，可快速导出和备份与恢复音乐播放器的设置功能与调试，支持通过json文件导出和代码直接导出，及其播放列表和自定义歌曲歌词文件",
                "[color:#4ecdc4]【优化改进】[/color]",
                "• 开发者弹窗样式与动效统一升级：「调整等级/经验」「调整签到天数」「自定义签到天数」三个开发者弹窗全面改为与阶段突破弹窗一致的效果——20px 大圆角容器、弹性弹出入场动画、关闭时淡出过渡动效，按钮统一为灰底取消 + 红橙渐变保存修改（替换原紫色渐变），整体视觉风格与系统弹窗保持一致",
                "• 签到经验与硬币发放提示改为静默：开启签到结果弹窗后，签到所得经验值与 PRE 硬币的发放提示不再额外弹出横条（改为静默发放并在结果弹窗中统一展示），其他功能（里程碑领取、邮件附件等）的横条提示不受影响",
                "[color:#ff6b6b]【修复问题】[/color]",
                "• 修复超过 60 级使用经验值补给卡导致页面系统直接卡死、补给卡被直接消耗但经验值未正确提高（吞物品不加经验）的问题：根因是补给卡等外部经验发放入口所使用的升级循环缺少等级上限保护——60 级满级时升级所需经验值为 0，循环条件永远成立并无限执行导致页面卡死，数据保存位于循环之后永远无法执行；现已为升级循环加入等级上限保护，满级状态下使用补给卡不再卡死，溢出经验值完整保留在账户内，同时经验值公式与主系统完全同步（支持阶段突破模式 120 级新公式）；经排查该 bug 可能在经验值系统更新后就已出现，现已向受影响用户发放补偿邮件",
                "• 修复百宝箱木鱼弹窗中点击重置按钮后「重置计数」确认弹窗被木鱼弹窗遮挡在下面、无法正常查看与点击的问题：根因是系统级确认弹窗层级（z-index 10002）低于百宝箱弹窗层级（z-index 100000）导致层级倒挂，现已将系统级确认/提示弹窗层级提升至 200000 并同步修正动态注入的同名样式，确保确认弹窗永远浮于所有业务弹窗最上层",
                "[color:#d45d79]【感谢支持】[/color]",
                "感谢您对 PRE Launcher 的持续关注和支持！本次版本更新为成长等级系统带来了全新「阶段突破」模式——从 60 级满级条目、双重确认弹窗、120 级新分档公式到每日签到奖励提升与双阶段独立里程碑，致力于为满级用户打造更长远、更有目标的成长体验；同时新增的木鱼攒功德、签到结果弹窗与音乐播放器导入导出等功能，也持续丰富着启动器的日常使用体验。",
                "对于本次修复的经验值补给卡问题给您带来的不便，我们再次诚挚致歉；如果您在使用过程中发现任何问题，或有任何想法和建议，欢迎通过 Github 仓库提交 Issue 与我们进行沟通。",
                "[color:black]© 2014-2026 PREAlmax. All rights reserved.[/color]"
            ]
        },
        {
            id: "devlog-20260918",
            title: "RC 3.0.2.5 开发日志",
            date: "2026-09-17",
            tag: "update",
            tagText: "公告",
            author: "GPY Games Studio - PREAlmax",
            ...
            images: [],
            content: [
                "今天我们发布了 RC 3.0.2.5 (c3) 常规版本更新！本次更新以「签到里程碑系统全新升级」为核心——每日签到里程碑奖励改为手动领取模式，签到成功后达成但未领取的里程碑会在结果提示中明确告知可领取数量；签到里程碑弹窗全新升级为 980px 宽度三列布局，七张里程碑卡片具备未达成/可领取/已领取三种状态并附脉脉动效与就地即时切换；新增「限时等级倍率提速活动」与经验倍率叠加系统，活动期间经验获取倍率提升至 ×1.25 / ×1.50 并与仓库经验值加成卡相乘叠加；每日签到卡片新增可领取奖励红点标签，成长等级卡片新增经验倍率条目并显示活动与加成卡叠加后的总倍率；同时邮件领取记录新增月份筛选，活动中心操作按钮布局重构，仓库加成卡激活提示优化，并修复了使用经验值加成卡后成长等级卡片经验倍率条目仍显示「无加成」的关键问题！",
                "[color:#d45d79]【新增功能】[/color]",
                "• 每日签到里程碑奖励改为手动领取：达到 7/15/30/60/90/180/365 天签到里程碑时不再随每日签到自动发放奖励，需要用户自行打开签到里程碑弹窗、点击对应里程碑卡片才能领取；签到成功后若存在已达成但未领取的里程碑，会在结果提示中明确告知可领取数量，引导用户前往「下一里程碑」手动领取",
                "• 签到里程碑弹窗全新升级：弹窗最大宽度由 580px 加宽至 980px，七个里程碑卡片由一行两个改为一行三个排列（中等宽度屏幕自动降为两列、手机端降为单列）；每张卡片具备三种状态——未达成（灰色，显示「还差 N 天」不可点击）、可领取（粉色描边高亮 + 脉脉动效 + 卡片底部悬浮「点击领取」按钮，整张卡片可直接点击）、已领取（紫色渐变背景 +「✓ 已领取」）；点击领取后弹窗不关闭，卡片就地即时切换为已领取状态，并弹出本次所得经验值、PRE Coin、特殊道具明细，若领取导致升级则同步提示升级",
                "• 签到里程碑弹窗底部新增状态提示栏：实时显示「当前连续签到：N 天」，当存在可领取奖励时右侧额外显示「🎁 有 N 个奖励可领取」，每次打开弹窗与每次领取后均按最新数据重新渲染",
                "• 每日签到卡片新增「有可领取奖励」红点标签：当存在已达成但尚未领取的签到里程碑时，每日签到卡片内「下一里程碑」标题右侧出现粉色渐变胶囊标签（礼物图标 + 呼吸闪烁动效，鼠标悬停提示具体可领取的里程碑天数），全部领取后标签自动消失，页面加载、签到完成、领取奖励后均实时刷新",
                "• 成长等级卡片新增「经验倍率」条目：在原「累计经验」「最高等级」两项右侧新增第三个统计项——使用经验值加成卡后显示当前叠加总倍率（如 ×1.05 / ×1.20，最多 3 张不同类型加成卡叠加后的总值）并在标签处显示「经验倍率 · 持续至 HH:MM」（取所有生效加成卡中最晚的到期时间）；未使用加成卡或加成全部到期后始终显示「无加成」；页面每 30 秒自动刷新一次，加成到期后无需刷新页面即自动恢复为「无加成」",
                "• 邮件领取记录新增筛选按钮，用户可根据需要筛选在特定月份内已领取的邮件记录（不包括因超出保存上限而被删除的记录）",
                "• 新增「限时等级倍率提速活动」：活动公告与活动中心特殊活动分类同步上线，活动时间 2026年9月17日 10:00 至 10月1日 23:59:59（UTC+8），共计 14 天；活动期间全账户成长等级经验获取倍率限时提升——第一阶段（第1-7天，9月17日-9月24日）经验倍率提升至 ×1.25，第二阶段（第8-14天，9月24日-10月1日）提升至 ×1.50，活动结束后自动恢复 ×1.00；活动固定倍率可与仓库「经验值加成卡」的提升倍率相乘叠加（总倍率 = 活动倍率 × 加成卡倍率），使用加成卡后仍遵循加成卡原有叠加规则（不同类型可同时生效、同类型不可叠加、最多同时 3 张、每张 30 分钟）；每日签到、签到里程碑、活动中心签到、邮件经验附件、经验值补给卡等所有经验获取途径均享受活动倍率加成，无需报名登录即自动生效",
                "• 经验倍率叠加系统上线：新增 getLevelBoostEventState、getEventExpMultiplier、getTotalExpMultiplier 三个核心函数，限时活动倍率以时间戳为唯一权威来源纯时间驱动判定（不依赖本地存储），每 30 秒自动校验阶段切换并刷新界面；成长等级卡片「经验倍率」条目同步升级为显示活动与加成卡叠加后的总倍率",
                "[color:#4ecdc4]【优化改进】[/color]",
                "• 签到里程碑数据层统一重构：七个里程碑奖励（经验值、PRE Coin、自选物品兑换卡、勤奋者勋章、坚持者勋章）整合为统一的 CHECKIN_MILESTONES 配置，弹窗展示与奖励发放共用同一份数据源，后续调整里程碑奖励只需修改一处配置",
                "• 签到里程碑领取规则细化：经验值与 PRE Coin 奖励按「连续签到周期」发放——每个周期内每个里程碑仅可领取一次，中途断签后领取记录自动重置，重新连续签到达到条件可再次领取；自选物品兑换卡与 180 天勤奋者勋章、365 天坚持者勋章等特殊道具保持终身仅发放一次，断签重领时只补发经验值与 PRE Coin，不会重复发放勋章道具",
                "• 每日签到经验奖励规则同步调整：每日签到只保留每日经验与不在里程碑弹窗内的 30 倍数循环奖励（如连续 120/150/210 天），7/15/30/60/90/180/365 天等全部弹窗里程碑经验与 PRE Coin 均改为手动领取后才发放，避免重复",
                "• 开发者模式各签到入口同步适配手动领取规则：「调整等级」直接设置连续签到天数、批量补签（含批量领取过程中发生断签）、清除本月签到等开发者操作均不再自动发放里程碑奖励，统一重置本轮领取记录并在完成提示中显示当前可手动领取的里程碑数量",
                "• 签到里程碑弹窗全部卡片尺寸固定：所有卡片统一为相同固定高度（按浏览器视口高度自适应，同行严格等高），奖励内容较多（如 180/365 天含自选物品兑换卡与勋章）时不再被内容撑长或拉大整行，卡片内部超长内容自动隐藏不外溢；「点击领取」按钮改为悬浮于卡片底部定位，不占用文档流，卡片在未达成/可领取/已领取三种状态间切换时高度完全不变，边框统一为 2px 避免状态切换时内容位移",
                "• 签到里程碑弹窗桌面端不再出现滚动条：移除弹窗固定最大高度与纵向滚动，三行卡片加上弹窗头部、底部提示栏的总高度始终小于浏览器视口高度；矮视口屏幕（高度 ≤800px）自动压缩卡片内边距与图标尺寸保证完整展示，仅手机端（宽度 ≤768px，单列七行）保留弹窗内滚动作为兜底，保证所有里程碑卡片均可查看",
                "• 活动中心操作按钮布局重构：「立即参与」与「活动规则」按钮从详情正文区移至顶部公告图片区最右侧，一行两个并排排列；未配置「立即参与」的活动（如限时等级倍率提速活动）仅显示「活动规则」按钮不留空位；活动未开始或已结束时「立即参与」按钮保持禁用灰色态；移动端自动缩小按钮尺寸仍保持一行排列",
                "• 成长等级卡片「经验倍率」条目显示升级：当限时活动与经验值加成卡同时生效时显示两者叠加后的合计总倍率（如 ×2.25），鼠标悬停可查看明细（限时活动 ×1.50，经验值加成卡 ×1.50）；仅活动生效时以粉色显示并标注当前阶段截止时间，仅加成卡生效时保持原橙色显示与到期时间，两者同时生效时标注「活动叠加至 HH:MM」",
                "• 仓库经验值加成卡激活提示优化：限时活动进行期间激活加成卡时，提示文案同时展示加成卡倍率与活动叠加后的合计总倍率（如「加成卡 ×1.50，叠加限时活动后合计经验获取 ×2.25」），活动未进行时保持原有提示格式",
                "[color:#ff6b6b]【修复问题】[/color]",
                "• 修复使用任意经验值加成卡后成长等级卡片「经验倍率」条目仍一直显示「无加成」、且激活提示中倍率也错误显示为 ×1.00 的问题：根因是仓库使用道具流程在函数开头就持有了一份仓库数据快照，激活加成卡时经验加成已正确写入本地存储，但随后扣减道具数量使用的是那份旧快照再次保存，把刚激活的经验加成数据整体覆盖丢失（道具却被正常消耗）；现已改为激活成功后重新读取最新仓库数据再扣减道具数量（普通加成卡与限时加成卡两条使用路径均已修复），经验加成激活后即时刷新成长卡片的经验倍率条目，激活提示中的总倍率也恢复正确显示",
                "[color:#d45d79]【感谢支持】[/color]",
                "感谢您对 PRE Launcher 的持续关注和支持！本次版本更新为签到里程碑系统带来了全新升级——从手动领取模式、弹窗三列三态卡片布局到统一数据层与领取规则细化，致力于为您打造更清晰、更可控的签到奖励领取体验；同时新增的限时等级倍率提速活动与经验倍率叠加系统、成长等级卡片经验倍率条目、邮件领取记录筛选以及活动中心布局重构，持续为您优化使用体验。希望能为您带来更丰富、更便捷的使用体验！",
                "如果您在使用过程中发现任何问题，或有任何想法和建议，欢迎通过 Github 仓库提交 Issue 与我们进行沟通。",
                "[color:black]© 2014-2026 PREAlmax. All rights reserved.[/color]"
            ]
        },
        {
            ...
            images: [],
            content: [
                "今天我们发布了 RC 3.0.2.4 (c3) 重大版本更新！本次更新以全新「启动器核心功能模式切换」为核心——首次启动自动弹出最高优先级的选择弹窗，支持在「完整启动器」与「浏览器首页导航」两种核心使用模式间自由切换，全新首页导航模式化身浏览器首页导航，提供中央大搜索框（六大搜索引擎一键切换）、常用网站快捷导航卡片、完整同步的音乐播放器、高度自定义抽屉面板、可拖拽可缩放窗口化播放器弹窗与双容器独立架构，同时新增搜索历史记录、必应联网联想、键盘导航与搜索历史管理弹窗等完整搜索体验，登录页全局设置弹窗全面改版，带来多项体验优化与问题修复！",
                "[color:#d45d79]【新增功能】[/color]",
                "• 新增启动器核心功能模式切换大功能：全新支持在「完整启动器」与「浏览器首页导航」两种核心使用模式间自由切换——首次启动启动器（或尚未做出选择时）自动弹出最高优先级的选择弹窗「您想如何使用本启动器？」（弹窗宽度加大且层级高于版本更新提示等其他弹窗，互不冲突），弹窗内提供「将启动器作为浏览器首页导航及其他辅助功能使用」与「使用完整的启动器功能（保持现状）」两个大按钮供点选，选中后点「确定」立即生效，也可点击「关闭」暂时跳过并于稍后在全局设置中再次选择（未做出选择前每次启动都会重新弹出提醒）",
                "• 登录页全局设置弹窗全面改版：外观升级为与活动公告弹窗一致的全屏样式（顶部粉色渐变条、标题竖线装饰、底部双渐变大按钮 取消/确定），移除原「语言」与「游戏服务器」条目，新增「切换功能」条目——以卡片单选形式在「浏览器首页导航模式」与「完整启动器模式」间切换，当前生效模式高亮显示，点击「确定」即时切换并弹出成功提示，切换过程完全不影响登录页的登录功能",
                "• 全新首页导航模式：切换至该模式后启动器化身浏览器首页导航——中央大搜索框支持百度/必应/谷歌/搜狗/360搜索/DuckDuckGo 六大搜索引擎一键切换（下拉选择并持久记忆，输入网址格式内容时直接打开对应网站），下方常用网站快捷导航卡片点击即在新标签页打开；顶栏提供「自定义」「全局设置」「进入启动器」三个入口，进入启动器后右下角出现悬浮「首页导航」按钮可随时一键返回",
                "• 首页导航模式完整同步音乐播放器：右下角音乐悬浮球一键唤出完整音乐播放器面板（与启动器内音乐播放器完全同源，播放列表、歌词、调音器、播放器设置等全部功能与数据实时同步共享），收起面板后播放不中断，底部悬浮控制条继续显示，与启动器内体验完全一致",
                "• 首页导航模式高度自定义：右侧滑出「自定义首页」抽屉面板，支持——搜索引擎单选切换、组件显隐开关（时钟问候/搜索框/快捷链接/音乐播放器四大组件独立控制）、快捷链接管理（添加/编辑/删除/左右移动排序，主界面卡片还支持直接拖拽换位）、背景设置（6 款预设渐变主题、自定义图片 URL、上传本地图片（≤4MB）、遮罩暗度 0~70% 与背景模糊 0~20px 双滑杆实时调节）、一键恢复全部默认设置（带二次确认）；所有配置自动写入 localStorage 持久化，刷新后完整恢复；首页中央时钟实时显示时间、日期星期与分时段问候语",
                "• 首页模式音乐播放器弹窗改为可拖拽可缩放窗口：右下角悬浮球唤出的音乐播放器弹窗不再固定锚定右下角，而是支持顶部标题栏整块区域作为拖拽手柄自由移动；四边 + 四角共 8 向 resize handles 实现多方向缩放，四角命中区域更大（18×18px）四边较薄（8px），hover 面板或缩放时自动显示 handles；弹窗设有最小尺寸限制 720×500，缩小触底自动顶住；拖拽/缩放位置尺寸自动写入 localStorage 持久化，下次打开恢复上次状态；页面 resize 时自动回拉面板确保仍在视口可见区域内；窗口化（windowed）前默认居中，首次拖拽或缩放自动进入 windowed 模式并锚定当前可视位置",
                "• 首页模式与游戏中心模式音乐播放器 UI 双容器独立：两种模式不再共用同一个 soundtrackArea DOM 容器——游戏中心使用 #soundtrackArea，首页导航使用独立的 #hnSoundtrackArea，各自互不干扰；soundtrack.js 保持单实例（音频/播放列表/设置状态共享），UI DOM 在两个容器间运行时迁移（appendChild 不重建），迁移通过 window.setSoundtrackContainer(id) 暴露给外部，切换模式时自动完成挂载，解决了首页打开面板后进入游戏中心播放器 UI 消失的历史问题；底部控制条（miniBar）始终挂载在 body 上，通过 _currentContainerId 追踪当前宿主容器可见性来决定显隐",
                "• 首页模式搜索框新增搜索历史记录：首次点击搜索框获得焦点时，下方自动划出建议面板，优先展示近 6 条历史记录（按当前输入内容实时过滤），历史去重后自动置顶，总上限 200 条，全部写入 localStorage（key homenav_search_history）持久化；每条历史右侧有 X 按钮，hover 时显示，点击即可单条删除",
                "• 首页模式搜索框新增必应联网联想：输入 1 个字符以上时，建议面板在历史记录下方展示必应公共联想接口（https://www.bing.com/AC）返回的联网联想结果（JSONP 跨域无需后端代理），中文 query 带 mkt=zh-CN&setlang=zh-CN；联网联想自动过滤与历史重复的条目，最多展示 8 条；请求加 220ms debounce + 2.5s 超时兜底，防止打字抖动",
                "• 首页模式搜索建议面板支持键盘导航：↑/↓ 键循环高亮建议项（历史和联想均可高亮），Enter 键执行当前高亮项（未高亮则正常搜索），Esc 键关闭面板并让输入框 blur；匹配到的关键词在历史和联想条目中以主题色加粗高亮显示",
                "• 首页模式新增「搜索历史管理」弹窗：建议面板底部提供「管理搜索历史记录」入口按钮，点击后弹出全屏居中的历史管理弹窗（fixed 覆盖浏览器视口，不局限于首页导航区域）；弹窗内按日期分组展示全部历史记录，顶部提供时间段多级筛选按钮（全部/近一个月/近一周/近一天），筛选切换即时刷新列表；支持每条历史单独删除（hover 显示 X）与一键清除全部（带二次确认，红色警示按钮，清除不可恢复提示）",
                "[color:#4ecdc4]【优化改进】[/color]",
                "• 音乐播放器拖拽/缩放动画流畅性：拖拽和缩放过程中临时禁用面板 CSS transition，让 DOM 跟随鼠标更流畅无卡顿，操作结束后再恢复过渡动画",
                "[color:#ff6b6b]【修复问题】[/color]",
                "• 修复首页模式下进入首页后首次打开音乐播放器弹窗空白、无内容的问题：根因是 soundtrackArea DOM 元素只在 switchToGameCenter 调用时通过动态 innerHTML 注入，首页导航首次启动时从未触发过该注入，导致 document.getElementById('soundtrackArea') 返回 null，挂载逻辑被跳过；现已通过首页独立容器 hnSoundtrackArea 在首页模式 DOM 构建阶段即时创建，不再依赖进入游戏中心才能渲染 UI",
                "• 修复首页模式下缩小音乐播放器弹窗后底部 miniBar 不出现的问题：根因是 closeMusicPanel 只移除了面板的 .open 类（让面板变透明），但 soundtrackArea 的 display 未设为 none，导致 soundtrack.js 的 isPlayerAreaVisible 通过 getBoundingClientRect 检测到宽高 > 0 仍认为播放器可见，miniBar 条件不满足；现已在关闭面板时显式设置容器 display:none，miniBar 轮询检测到不可见后自动显示",
                "• 修复首页模式打开音乐播放器后退出首页进入游戏中心时，游戏中心音乐播放器 UI 直接消失的问题：根因是两套模式此前共用同一个 soundtrackArea DOM 容器，首页把它搬去自己的面板里初始化渲染，游戏中心的 innerHTML 模板又创建了一个新的空 soundtrackArea，但 soundtrack.js 的 initialized 标志已为 true 导致不会重新渲染，游戏中心看到的是空容器；现已改为双容器独立架构 + 运行时 DOM 迁移，两种模式各自拥有独立 DOM 容器，互不干扰",
                "• 修复搜索历史管理弹窗被限制在首页导航区域内显示而非覆盖全屏的问题：根因是弹窗原先使用 position:absolute 建在首页导航 overlay 内部，被父级 stacking context 限制；现已改为 position:fixed + z-index:9999 并直接挂到 document.body，无论首页导航是否打开自定义抽屉，管理弹窗始终覆盖整个浏览器视口居中显示",
                "• 修复搜索建议面板中点击单条历史的删除按钮（X）后，删除操作未生效反而触发搜索并在新标签页打开的问题：根因是事件委托中 .hn-suggest-item 的判断排在 .hn-suggest-del 之前，点击 X 按钮时先被 item 分支命中执行了 onSuggestItemClick 并 return，永远走不到删除逻辑；现已将 .hn-suggest-del 判断移到最前面 + item 判断加 !e.target.closest('button') 双重保护 + 删除时加 e.preventDefault() 彻底消除冒泡误触发",
                "[color:#d45d79]【感谢支持】[/color]",
                "感谢您对 PRE Launcher 的持续关注和支持！本次版本更新带来了全新的启动器核心功能模式切换——首次启动可选择将启动器作为浏览器首页导航使用，全新首页导航模式提供大搜索框、快捷网站导航、完整同步的音乐播放器、高度自定义抽屉、可拖拽可缩放窗口化播放器，以及搜索历史记录、必应联网联想、键盘导航与搜索历史管理弹窗等完整搜索体验。希望能为您带来更丰富、更便捷的使用体验！",
                "如果您在使用过程中发现任何问题，或有任何想法和建议，欢迎通过 Github 仓库提交 Issue 与我们进行沟通。",
                "[color:black]© 2014-2026 PREAlmax. All rights reserved.[/color]"
            ]
        },
        {
            ...
            images: [],
            content: [
                "今天我们发布了 RC 3.0.2.3 (c3) 常规版本更新！本次更新以音乐播放器歌词系统与调音器为核心——全新 LRC 歌词滚动显示、自定义歌词文件上传、内置调音器与均衡器面板（9 种预设混音 + 7 段 EQ + 低音/高音/立体声/主音量调节），并新增控制条收起/展开、快速跳转、重置播放器设置等实用功能，同时重构开发者模式入口为隐藏式 5 次点击 Logo 触发，带来十余项体验优化与问题修复！",
                "[color:#d45d79]【新增功能】[/color]",
                "• 音乐播放器新增歌词滚动显示功能：控制区音量条左侧（静音按钮左侧）新增「显示歌词」开关按钮（CC 字幕样式图标），点击开启后歌词展示区出现在唱片与歌曲信息之间，随播放进度逐行高亮当前句并平滑滚动保持居中；歌词区采用上下边缘渐隐遮罩与隐藏滚动条设计，当前行以主题渐变色高亮并轻微放大，完整适配暗色模式；按钮激活态与随机/循环按钮一致显示高亮圆点，开关状态随播放器设置写入 localStorage 持久化，刷新页面后自动恢复",
                "• 音乐播放器新增 LRC 歌词文件路径绑定：全部内置曲目新增 lrc 歌词字段，歌词文件统一存放于 sounds/lrc/ 目录并按「歌曲文件名.lrc」一一对应（中文、空格、特殊符号歌名均可正确加载），文件头部「如何添加音乐」说明同步补充歌词接入方式——为新曲目追加一行 lrc 字段即可启用歌词，未绑定歌词的曲目在歌词开启后显示「暂无歌词」占位提示，不弹错误",
                "• 音乐播放器新增「添加自定义歌词文件」设置条目：播放器设置侧边栏在「播放器组件版本」上方新增该条目（暖橙渐变图标），点击后右侧内容区先显示「请先选择一个播放列表（不包括默认列表）」提示，通过下拉选择器从所有已建立的自定义播放列表中选择目标列表（默认列表不参与选择）；若检测到播放器除默认列表外没有其他播放列表，则直接显示「当前没有可选择的播放列表，请先创建一个自定义播放列表」引导文案，选定列表后内容区展示其中已导入的全部自定义歌曲卡片（渐变封面、歌名、艺人，附「已添加歌词 / 添加歌词」状态标签），空列表时显示空状态提示引导先导入歌曲",
                "• 音乐播放器新增 LRC 歌词上传弹窗：点击歌曲卡片弹出居中弹窗（显示目标歌曲名称与艺人），支持点击选择与拖拽两种方式上传歌词文件，弹窗内明确标注仅支持 .lrc 格式——文件选择框限定 .lrc 扩展名且读取前进行强制格式校验，选择非 LRC 格式文件时直接禁止并弹出提示；选择成功后显示文件名与一键移除按钮，「确定」按钮在未选择有效文件时禁用，点击遮罩空白处或「取消」可关闭弹窗",
                "• 音乐播放器导入歌词即时生效与持久化：点击「确定」后歌词文本写入对应曲目并立即生效——若该曲目正在播放，歌词显示区马上重新加载并随播放进度滚动展示；导入的歌词文本随播放列表结构一并写入 localStorage 持久化保存，刷新页面后自动恢复，已导入歌词的歌曲在面板中持续显示「已添加歌词」标记，可随时再次点击歌曲重新上传覆盖",
                "• 音乐播放器底部控制条新增「隐藏/展开」按钮：底部悬浮控制条最右侧新增收起按钮（下拉箭头图标），点击后控制条以从上到下的平滑滑动动画暂时移出页面隐藏，不再遮挡页面内容",
                "• 音乐播放器新增页面底部小凸起展开图标：控制条被手动隐藏后，页面最底部中心处出现一个小凸起图标（fixed 固定定位、紧贴浏览器底边，不受页面滚动影响始终固定在最下方），点击该图标即可重新从底部弹出控制条；此隐藏为暂时性行为，刷新页面后控制条自动恢复显示",
                "• 音乐播放器控制条设置新增「在控制条内显示收起/展开按钮」开关：播放器设置 →「调整播放器控制条设置」面板在原开关下方新增该开关（默认开启）——开启时控制条最右侧正常显示隐藏/展开按钮，关闭后按钮不再显示；关闭开关时会自动清除控制条的手动隐藏状态使其立即恢复显示，避免出现既无按钮也无凸起图标导致控制条无法找回的情况；开关状态通过 localStorage 持久化，刷新后自动恢复",
                "• 开发者模式入口重构：系统设置页危险区域中原「进入/退出开发者模式」条目不再对外显示（已删除），改为隐藏式触发——在登录页 / 游戏中心 / 系统设置页左上角 PRE Launcher 标题左侧的图标 Logo 上连续点击 5 次（两次点击间隔超过 2 秒自动重新计数）即可触发：未开启开发者模式时弹出与原先一致的「请输入开发者密码」弹窗（支持回车提交），输入密码验证成功后弹出「验证成功」弹窗并自动刷新启用；已开启开发者模式时则先弹出「是否要退出开发者模式？」确认弹窗，点击确认按钮后才会退出开发者模式，取消 / Esc 可安全退出；触发弹窗按需动态创建（结构与原弹窗完全一致，支持 Enter 确认 / Esc 取消），登录页未登录状态下开启的开发者模式以 __global__ 全局键持久化，登录后商店 / 仓库 / 每日任务等开发者功能自动继承生效",
                "• 音乐播放器新增「重置对播放器的设置」功能：播放器设置侧边栏最下方新增该条目（红粉渐变回转图标），点击后右侧内容区显示重置操作面板，提示可重置的播放器功能包括但不限于自定义播放列表、自定义歌词文件、自定义曲目背景图片、对播放器组件的功能调整（组件位置与大小、显隐状态、控制条设置、播放样式）等内容；每个条目均以复选框形式独立勾选，也提供「清除所有设置」按钮一键全清——点击「清除所选设置」或「清除所有设置」后均会弹出二次确认弹窗（按清除范围显示「是否要清除「条目名称」数据？」或「是否要清除全部数据？清除后将立即生效，不可恢复」），点击「确定清除」后才执行清除操作，取消或点击遮罩 / 按 Esc 可安全退出；清除自定义播放列表时同步清理导入歌曲的本地音频数据（IndexedDB）并自动切回默认列表，清除自定义歌词后立即刷新歌词显示区，清除背景图后即时移除当前曲目背景，清除组件调整后布局 / 显隐 / 控制条 / 播放样式全部恢复系统默认并解锁被律动样式锁定的布局条目，清除完成弹出成功提示",
                "• 音乐播放器控制条新增「快速跳转」功能：播放器设置 →「调整播放器控制条设置」面板新增开关「离开音乐播放器后点击控制条当前播放曲目快速跳转」（默认开启，位置定位箭头图标）——开启后，离开音乐播放器（切换到游戏中心其他条目 / 页面）时点击底部控制条左侧当前播放的曲目标题区域，即可快速跳转回音乐播放器页面（在播放器设置界面点击则直接返回播放器主界面），曲目区域同步显示手型光标、悬停标题高亮与「点击返回音乐播放器」提示；关闭后点击曲目标题不再跳转；开关状态通过 localStorage 持久化，刷新后自动恢复，重置播放器设置时一并恢复为默认开启",
                "• 音乐播放器新增调音器：在播放器主界面右侧音量条右侧新增调音器按钮（滑块图标，激活时高亮），点击后在 stk-main 内从右侧滑出调音器面板（关闭按钮 / ESC / 点击面板外区域可关闭，面板从 stk-main 内右侧向内滑入，不遮挡外部内容）；面板内包含 9 种预设混音（Flat / Bass / Treble / Vocal / Rock / Pop / Classical / Jazz / Electronic，激活预设渐变高亮）、7 段 EQ 均衡器（60Hz / 160Hz / 420Hz / 1kHz / 2.4kHz / 6kHz / 12kHz，dB ±12，立式滑块实时显示增益值）、额外调节区（低音增强 / 高音增强 / 主音量 50%~150% / 立体声 -100%~100% 居中显示 + 左右快速按钮）与快速操作按钮（应用预设 / 全部重置）；底层重构音频链路为共享图 srcNode → preGain → 7 段 EQ → 低音 shelf → 高音 shelf → StereoPanner → postGain → AnalyserNode → destination，调音器与律动可视化共用同一 AudioContext；所有调音器状态（EQ 各段增益、低音 / 高音 / 主音量 / 立体声 / 当前预设）随播放器设置一并写入 localStorage 持久化，刷新后自动恢复；「重置播放器设置」勾选清除组件调整时调音器一并恢复 Flat 预设默认值",
                "• 音乐播放器重构音频链路：原有 AudioContext 仅在律动样式开启时懒创建（srcNode → AnalyserNode → destination），现改为按需创建的共享链路，首次创建触发条件扩展为「律动样式开启」或「打开调音器」（均需用户手势，符合浏览器自动播放策略）；AnalyserNode 仍用于律动可视化，EQ / 低音 / 高音 / StereoPanner 节点始终串联，调音器关闭时节点为 flat（不改变原音频），不影响原生发声路径",
                "• 音乐播放器修复：控制条设置中「在控制条内显示收起/展开按钮」开关文本左侧的图标缺失问题（原 fa-chevrons-up-down 图标名在 Font Awesome 6.0.0 中不存在导致不渲染），替换为与控制条收起按钮一致的 fa-chevron-down 图标",
                "[color:#4ecdc4]【优化改进】[/color]",
                "• 音乐播放器歌词加载链路增强：歌词显示逻辑升级为「内联歌词文本 + 歌词文件路径」双通道支持，曲目携带导入歌词时直接解析渲染（支持多时间戳行、元数据标签与 offset 偏移），否则按原有文件路径加载，两种来源互不干扰",
                "• 音乐播放器底部控制条显隐状态统一管理：控制条手动隐藏标记与显示条件、收起/展开按钮显隐、底部凸起图标显隐在同一处统一同步，与设置模式进出、页面切换、播放器区域显隐轮询检测天然兼容，各场景下状态切换准确无冲突",
                "• 音乐播放器歌词文件加载健壮性优化：歌词文件按路径缓存避免重复请求，加载优先 fetch 并内置 XHR 回退以兼容本地 file:// 打开场景，http 下请求 404（文件不存在）时判定为确定缺失不再重复回退；解析层支持多时间戳行 [mm:ss.xx][mm:ss.xx] 与小数位精度（.xx / .xxx）、元数据标签与 [offset:...] 全局时间偏移修正，纯时间戳空行渲染为间奏留白；切歌与启动恢复上次曲目时自动加载对应歌词，加载期间切换曲目自动作废过期请求结果，防止歌词串曲",
                "• 音乐播放器「显示歌词」按钮位置优化：按钮由随机/播放/循环控制按钮行内移出，调整至下方音量条最左侧（静音按钮左侧），控制按钮行恢复精简布局，按钮位置与常见播放器布局习惯保持一致",
                "[color:#ff6b6b]【修复问题】[/color]",
                "• 修复音乐播放器「显示歌词」按钮图标不显示的问题：项目引用的 Font Awesome 6.0.0 图标库中不存在 fa-captions 图标（该图标于 6.1+ 版本才加入）导致按钮渲染为空白圆形，已更换为 FA 5/6 均内置的 fa-closed-captioning 字幕图标",
                "[color:#d45d79]【感谢支持】[/color]",
                "感谢您对 PRE Launcher 的持续关注和支持！本次版本更新为音乐播放器带来了完整的歌词系统——从 LRC 文件路径绑定、歌词滚动显示到自定义歌词文件上传，致力于为您打造沉浸式的音乐聆听体验；同时新增的调音器与均衡器面板、控制条收起/展开、快速跳转、重置播放器设置等功能，以及重构的开发者模式隐藏入口，持续为您优化使用体验。希望能为您带来更丰富、更便捷的使用体验！",
                "如果您在使用过程中发现任何问题，或有任何想法和建议，欢迎通过 Github 仓库提交 Issue 与我们进行沟通。",
                "[color:black]© 2014-2026 PREAlmax. All rights reserved.[/color]"
            ]
        },
        {
            ...
            images: [],
            content: [
                "今天我们发布了 RC 3.0.2.2 (c3) 重要版本更新！本次更新的主角是全新重置改版的「音乐播放器」：游戏中心多级菜单新增音乐播放器入口，全部 UI 渲染、样式注入与运行逻辑封装于专属文件 js/soundtrack.js，以独立模块承载双栏播放界面、播放列表管理、本地音频导入、专辑浏览、按曲目独立自定义背景与随音乐律动的播放样式可视化等完整能力，全部设置本地持久化保存；同时抽卡模拟器新增进入声明弹窗，并附带 PREL_Server.bat 本地服务器一键启动脚本、百宝箱与游戏中心卡片视觉全面升级，以及音乐播放器十余项体验优化与问题修复！",
                "[color:#d45d79]【新增功能】[/color]",
                "• 抽卡模拟器进入声明弹窗：从游戏中心多级菜单选择「抽卡模拟器」后，先弹出声明弹窗提示本模拟器仅供娱乐、概率纯属猜测与官方无关、不会影响游戏数据；弹窗内含「今日内不再显示」复选框，勾选并确认后当日再进入时直接跳过（UTC+8 次日自动恢复）",
                "• 全新「音乐播放器（原游戏原声带）」功能重置改版：游戏中心多级菜单新增「音乐播放器」条目（位于「抽卡模拟器」下方），点击进入独立音乐播放界面；播放器的全部 UI 渲染、样式注入与运行逻辑封装于专属新文件 js/soundtrack.js，模块独立、不与其他功能耦合，后续新增曲目仅需在曲目数组中配置音频文件路径即可",
                "• 音乐播放器采用双栏布局：左侧为播放列表面板（曲目序号、渐变封面图标、曲名、艺术家、时长，当前播放曲目显示跳动的均衡器动画，列表底部附音频资源放置指引）；右侧为主播放区（旋转黑胶唱片、曲目与专辑信息、可拖拽进度条、播放/暂停、上一首/下一首、随机播放、循环模式切换（关闭/列表循环/单曲循环）、音量滑块与静音按钮、右上角设置按钮、右下角折页入口）",
                "• 音乐播放器核心功能：基于 HTML5 Audio，支持进度条点击与拖拽跳转（Pointer Events，同时兼容鼠标与触摸）、播放缓冲状态提示、音源加载失败时自动跳过下一首并弹出提示、全部音源缺失时给出放置指引；音量、静音、随机/循环模式及上次播放曲目均通过 localStorage 持久化，重新进入自动恢复设置",
                "• 音乐播放器自定义背景：右下角折页为背景入口，鼠标悬浮折页时当前曲目背景以 18% 淡透明预览，点击折页后背景完全铺满整个播放区（再次点击空白区域或按 Esc 收起）；支持为每首曲目单独上传本地图片作为专属背景——背景按曲目独立存储、互不影响，切换歌曲自动展示对应背景（单张背景仅对应一首曲目），设置面板内含 16:9 实时预览、3MB 图片大小限制校验、保存与一键清除功能",
                "• 音乐播放器新增「调整播放器组件显示状态」功能：在播放器设置中可独立开关唱片封面与歌曲名称/艺人信息的显示（进度条与控件按钮等核心功能区始终保持可见不可隐藏），设置通过 localStorage 持久化，刷新后自动恢复",
                "• 音乐播放器新增「调整播放器组件位置和大小」功能：提供九宫格位置预设（左上/左中/左下/上方/居中/下方/右上/右中/右下）与70%~130%缩放滑块，实时预览、一键恢复默认，所有布局设置通过 localStorage 持久化",
                "• 音乐播放器新增侧边栏收起/展开功能：播放列表右上角新增收起按钮，点击后侧边栏平滑收缩隐藏，内容区自动向左扩展铺满；主播放区左上角同步出现展开按钮（未收起时不显示），点击恢复侧边栏；收起状态通过 localStorage 持久化",
                "• 音乐播放器新增播放/暂停音量淡入淡出效果：点击暂停时音量在500ms内平滑降至静音后再暂停，点击播放时音量从零缓慢回升至正常音量（easeInOutQuad缓动曲线），给用户舒缓过渡体验；该效果仅作用于 audio.volume 底层音量，不影响音量条 UI 显示",
                "• 音乐播放器设置交互全面重构（设置弹窗改为侧边栏设置模式）：播放器设置入口改为播放列表标题右侧的齿轮按钮，点击后整个播放器进入「播放器设置」模式——左侧边栏平滑切换为设置条目菜单（附图标与说明文字），右侧内容区停止显示唱片与歌曲信息、改为展示所选举目对应的功能面板；侧边栏标题同步切换为「播放器设置」并显示副标题，附返回按钮可随时一键返回播放列表，所有更改即时生效并通过 localStorage 持久化",
                "• 音乐播放器新增底部播放控制条：正在播放音乐时进入播放器设置，页面底部会从下到上平滑滑出一个四角圆角的悬浮控制条，保证调整设置期间音乐不中断且始终可控——内含当前曲目名称与艺术家、随机播放、上一曲、播放/暂停（渐变圆形主按钮）、下一曲、循环模式、可拖拽进度条与实时时间显示、音量滑块与静音按钮，全部控制与主播放界面双向同步（任一处操作另一处状态即时更新），进度条支持点击与拖拽跳转（Pointer Events，兼容鼠标与触摸），窄屏下自动隐藏曲目信息与音量区并收紧间距",
                "• 音乐播放器新增播放列表管理系统：播放列表头部新增「更多」（…）按钮，点击弹出多级下拉菜单，内含「新建播放列表」与「删除播放列表」两个条目；点击「新建播放列表」后立即创建并自动切换到一个空的全新播放列表（自动去重命名，如「新建播放列表 2」），侧边栏即时刷新为空列表状态",
                "• 音乐播放器播放列表标题文本改为可点击：点击标题弹出切换菜单，菜单内列出目前所有已建立的播放列表（当前列表带 ✓ 选中标记并显示曲目数量），点击任意列表即可立即切换（切换时自动暂停当前播放避免状态错乱）；菜单底部附「新建播放列表」快捷入口，方便随时追加新列表",
                "• 音乐播放器所有播放列表支持重命名：切换菜单中每个播放列表条目悬浮时显示重命名（铅笔）按钮，点击后该条目原地切换为行内输入框（自动聚焦并全选），输入新名称后按 Enter 或点击 ✓ 按钮确认、按 Esc 或点击 ✕ 按钮取消；重命名实时同步至侧边栏标题并持久化保存",
                "• 音乐播放器新增删除播放列表确认机制：点击「删除播放列表」展开二级子菜单，列出目前所有已建立的播放列表（各条目以红色警示样式显示并附带曲目数）；选择其中一个列表后先弹出居中确认弹窗（含警示图标、列表名称与歌曲数提示、「取消/确定删除」按钮），点击确定按钮后才执行删除操作；内置默认播放列表受保护不可删除（置灰显示并提示），删除列表时其中导入歌曲的音频数据（IndexedDB）同步清理，删除的若为当前活动列表则自动切回默认列表",
                "• 音乐播放器自定义播放列表新增「导入歌曲」功能：切换到新建的自定义播放列表后，侧边栏曲目区上方显示渐变色「导入歌曲」按钮，点击后可一次选择多个本地音频文件（支持 mp3/ogg/wav/flac/m4a/aac/opus 等格式）导入当前播放列表；导入时自动按「艺人 - 歌名」格式解析文件名、循环分配渐变封面图标，导入完成后立即出现在列表中且可直接点击播放；空播放列表时显示唱片图标空状态提示引导导入",
                "• 音乐播放器导入歌曲持久化存储：导入的音频二进制数据通过 IndexedDB（soundtrackAudioStorage）持久化保存，播放列表结构（列表名称、曲目信息、当前激活列表）通过 localStorage 持久化，刷新页面或重启后自动恢复全部自定义播放列表与导入歌曲，且可直接播放；导入曲目在音源尚未恢复完毕时点击播放会先显示「正在读取音频数据…」并自动接续播放，避免竞态导致的播放失败",
                "• 音乐播放器新增「播放器控制条设置」条目：播放器设置侧边栏新增第四个设置条目，内含「离开音乐播放器后始终显示底部控制条」开关（默认开启）——开启时正在播放音乐的前提下切换到页面其他条目仍会显示底部控制条，关闭后离开播放器即不再显示，更改即时生效并通过 localStorage 持久化，刷新后自动恢复",
                "• 音乐播放器新增「专辑浏览」功能：播放列表头部「更多」（…）按钮左侧新增圆形「专辑」按钮（唱片图标），点击后右侧内容区由播放器组件整体切换为专辑总览区——以一行最多五张的响应式网格（≤1100px 自动降为四列、≤880px 三列、≤600px 两列）展示圆角专辑块状按钮（渐变封面、专辑名与曲目数、悬浮上浮动效），并在内容区顶部显示专辑数量提示；此时左侧侧边栏不显示曲目、改为居中提示「请在右侧先选择一张专辑」；点击任意专辑卡片后右侧恢复播放器组件并自动载入该专辑全部曲目，侧边栏同步切换为该专辑的曲目列表，内容区左上角出现「返回专辑列表」按钮可随时返回重新选择专辑（按 Esc 键同样可逐级返回），再次点击专辑按钮则完全退出专辑浏览并恢复进入前的播放列表状态",
                "• 音乐播放器新增专辑数据配置块 SOUNDTRACK_ALBUMS：专辑定义集中在 soundtrack.js 独立代码块中，制作新专辑仅需填写专辑 id、名称、简介、主题渐变色以及曲目 id 列表（自动按 id 从所有播放列表曲库中筛选并按序组装、自动去重），无需重复填写曲目信息，新建专辑只需在数组末尾追加一条配置即可，后续新增专辑零门槛",
                "• 音乐播放器新增「调整播放器音乐播放样式」设置条目：播放器设置侧边栏新增第五个设置条目（粉色渐变波形图标），点击后右侧内容区显示样式选择面板，顶部为实时预览小画布、下方为 2×2 样式选项卡片，共提供四种选择——「默认样式」（无律动效果）、「音浪」（内容区下侧 48 根蓝紫至玫红渐变圆角频谱柱随节拍实时跳动，频谱按低中高频非线性映射并附底部光晕）、「波纹」（内容区中心呼吸光晕随低音冲击生成彩色涟漪圈向外扩散消散，中心圆点随低音缩放）、「流光波浪」（内容区下侧三层相位错开的蓝紫粉声波曲线随旋律起伏，波峰带发光描边）；律动效果基于 Web Audio API AnalyserNode 实时分析正在播放音乐的频谱能量（分低/中/高/整体频段、指数平滑快起慢落），暂停或未播放时自动切换为平静呼吸动画，浏览器不支持 Web Audio 时自动降级为时间驱动的模拟律动；所选样式通过 localStorage 持久化，刷新后自动恢复，设置面板内选择样式即时预览",
                "• 新增 PREL_Server.bat 本地服务器一键启动脚本（该脚本暂不提供在 Github 项目内）：由于音乐律动可视化依赖的 Web Audio 音频分析接口在 file:// 协议直开页面时会被浏览器安全策略限制，新增项目根目录下的 PREL_Server.bat（全英文内容、纯 ASCII 编码避免代码页乱码），双击即可自动检测 Python（优先 python 命令、回退 py 启动器）、自动探测 8000–8020 空闲端口、自动定位 Google Chrome（依次检查 Program Files / Program Files (x86) / LocalAppData 三个标准路径，不使用其他浏览器），随后启动仅监听本机 127.0.0.1 的 Python HTTP 服务并延迟 3 秒自动用 Chrome 新窗口打开项目页面（保证服务先就绪），脚本窗口保持显示服务地址与项目路径，关闭窗口即停止服务",
                "[color:#4ecdc4]【优化改进】[/color]",
                "• 百宝箱卡片样式全面优化：卡片尺寸缩小，Logo移至卡片右下角并新增多种渐变效果（粉紫/青蓝/青绿/粉黄/粉青/粉红/紫蓝七色），名称置于左上角，按钮置于左下角；鼠标悬浮时卡片上移、Logo弹性旋转放大并外发光、按钮光扫、标题渐变文字",
                "• 百宝箱排版改为一行三列Grid布局：由纵向单列改为三列等宽排列（响应式：≤1200px降为两列、≤768px降为单列）",
                "• 游戏中心卡片新增hover动效：与百宝箱一致，卡片上移+渐变蒙层浮现、Logo弹性旋转+外发光、按钮光扫效果+放大、标题紫蓝渐变文字",
                "• 音乐播放器主播放区宽度优化：取消播放区最大宽度限制，右侧内容区随浏览器宽度自动拉伸铺满，贴近浏览器边框的同时保留合理边距，大屏下视觉更舒展",
                "• 音乐播放器折页背景交互体验优化：折页按钮固定于右下角不再随动效移动，背景预览采用淡透明渐变过渡，悬浮预览与点击完全展开两个阶段衔接平滑，折页翻角动效与整体紫粉主题风格统一",
                "• 音乐播放器组件自适应对比度优化：展开背景图后，播放器的歌曲信息、进度条、控制按钮、音量区等组件自动添加半透明深色蒙层（rgba(0,0,0,0.35)+阴影），文字切换为白色/亮色并附文字阴影，保证在任意亮度的背景图上均清晰可辨",
                "• 音乐播放器折页展开状态持久化：点击折页展开自定义背景后，刷新页面自动恢复展开状态（仅当当前曲目确实已设置背景图时才恢复，避免无效展开）；折页图标在展开/收起状态间自动切换（图片图标↔下拉箭头），展开后折页保持可见不再隐藏",
                "• 音乐播放器底部控制条全局化与固定定位增强：底部控制条不再局限于播放器设置模式——正在播放音乐时切换到页面其他任意条目（游戏中心子页面如休闲小游戏/抽卡模拟器，或顶部导航的百宝箱、仓库等其他页面），控制条同样会从底部滑出供随时操控音乐；控制条改为 fixed 固定定位并挂载至 body（脱离播放器容器，不受页面切换隐藏与父级样式影响），始终固定显示在浏览器视口最下方且四周与浏览器边框保持 16px 间距，页面上下滚动时位置纹丝不动；返回音乐播放器普通界面时控制条自动收起隐藏（避免与主播放区控件重复），进入设置模式或离开播放器页面时自动恢复显示，暗色主题与响应式断点均已适配",
                "• 音乐播放器侧边栏头部按钮布局优化：标题区现显示当前播放列表名称并附带下拉箭头指示可点击，副标题曲目计数随切换列表实时更新；「更多」按钮与设置按钮、收起按钮纵向对齐统一为 28px 圆形样式，悬浮渐变高亮反馈一致",
                "• 音乐播放器下拉多级菜单交互优化：子菜单支持鼠标悬浮展开与触摸点击展开双模式，菜单挂载至 body 并采用 fixed 定位跟随锚点按钮（自动规避视口边界、页面滚动/缩放时实时重新定位），点击菜单外部区域、再次点击触发按钮或按 Esc 键均可关闭；进入播放器设置模式时自动收起全部菜单并隐藏「更多」按钮与导入栏",
                "• 音乐播放器多级菜单与确认弹窗视觉适配：下拉菜单、行内重命名输入框、删除确认弹窗均采用与播放器一致的紫粉渐变主题与圆角卡片样式（附带弹出动画），并完整适配暗色模式；删除确认弹窗使用半透明模糊遮罩，点击遮罩空白处等同取消",
                "• 音乐播放器专辑总览态背景策略优化：进入专辑总览（尚未选择专辑）时，每首歌曲的自定义背景图与背景暗化蒙层被强制隐藏，右侧内容区统一呈现白色（暗色模式为深色）干净底色以突出专辑卡片网格；选定专辑进入曲目播放后自定义歌曲背景自动恢复正常显示，专辑浏览与播放器设置两种模式互斥、进入任一方都会清理另一方状态",
                "• 音乐播放器律动样式位置固定优化：启用播放样式律动效果后，「音浪」频谱柱与「流光波浪」声波均固定锚定在内容区下侧区域（频谱基线贴底、柱高与波幅封顶不侵入上半部分），「波纹」涟漪则固定以内容区正中心为圆心扩散；律动画布改为显式 100% 填充主播放区并适配 devicePixelRatio 高清屏渲染，在高分辨率屏幕上不再出现只绘制在左上角一小块区域的问题",
                "• 音乐播放器律动样式与组件布局互斥策略优化：启用任意律动样式（音浪/波纹/流光波浪）后，「调整播放器组件位置和大小」设置条目自动变为灰色不可点击状态（降低透明度、去色、禁止光标并隐藏箭头），条目说明同步改为「律动样式启用时已锁定（组件固定为系统默认位置）」以明确告知禁用原因，同时播放器组件自动重置回系统默认居中位置与 100% 缩放，九宮格、缩放滑块与重置按钮的操作也同步拦截；切回「默认样式」后该条目立即恢复可点击",
                "• 音乐播放器音频分析链路健壮性优化：律动可视化的 Web Audio 分析链路改为按需创建——仅在启用律动样式时才接管音频元素（选择默认样式时完全不触碰音频，保持原生发声路径），且建立顺序调整为先接通「分析器→扬声器」输出链路、最后才接管音频元素，任何一步失败均不影响正常发声并在下次播放时自动重试；分析链路被浏览器自动播放策略挂起时，播放期间每 2 秒自动尝试恢复上下文",
                "[color:#ff6b6b]【修复问题】[/color]",
                "• 修复每日「N」句名言卡片一直显示「加载中」的问题：DOM中存在两个重复的id=\"quoteContent\"（页面时钟与百宝箱各一个），getElementById只返回第一个，导致百宝箱名言容器永远无法更新；已为百宝箱名言容器分配独立ID消除冲突",
                "• 修复音乐播放器折页悬浮时动效在鼠标附近反复抖动、来回回弹的问题：折页动效原先绑定在自身会移动的元素上，移动后鼠标命中区离开光标触发 mouseleave、元素弹回又触发 mouseenter，形成「移动→丢失→回弹→再触发」的无限反馈循环；已将鼠标检测区（固定在右下角的 hitzone）与视觉动效层彻底分离，视觉层位移不再影响命中检测，抖动消除",
                "• 修复音乐播放器单曲循环模式图标未正确显示的问题：项目使用 FontAwesome 6.0.0 CDN，该版本不存在 fa-repeat-1 图标，导致单曲循环时仍显示为普通循环图标；已改用 fa-repeat + CSS ::before 叠加紫色渐变圆形「1」角标方案，单曲循环状态视觉清晰可辨",
                "• 修复音乐播放器设置弹窗中选择图片后预览区仍残留「当前曲目尚未设置背景」提示文字的问题：CSS缺少 .stk-bg-preview.has-image .stk-bg-preview-empty { display: none } 隐藏规则，空文字占位符未被隐藏；已补充隐藏规则",
                "• 修复音乐播放器展开折页背景后点击播放器任意按钮/控件时背景自动消失的问题：原先展开后点击 stk-main 非折页区域会收回背景，与用户预期不符；已移除该监听器，仅保留再次点击折页处或按 Esc 键两种收回方式",
                "• 修复音乐播放器展开折页背景后折页按钮不可见导致用户无法点击收回的问题：展开后折页 opacity 被设为 0 且 hitzone 的 pointer-events 被禁用，用户只能靠 Esc 收回；已保持展开后折页可见（opacity 0.85）且始终可点击，同时展开时图标自动切换为下拉箭头提示可收起",
                "• 修复底部播放控制条随页面滚动上下移动的问题：控制条最初采用 absolute 绝对定位挂载在播放器容器内部，切换到其他页面后因父级隐藏而无法显示，页面滚动时还会随内容一起移动；已改为 fixed 固定定位并将控制条节点挂载至 document.body，配合轻量轮询实时检测播放器区域显隐状态，确保控制条固定于视口底部不随滚动位移，且各页面切换时显隐及时准确",
                "• 修复启用音乐律动可视化后播放歌曲没有声音、但进度条仍正常走动的问题：Web Audio 的 MediaElementSource 节点一旦创建就会永久接管音频元素输出，原先音频分析链路在任何播放时都会无条件创建且顺序为「先接管音频元素、后连接扬声器」，链路中途被浏览器自动播放策略打断时音频会被接入未接通的死路导致无声；已改为仅在启用律动样式时才创建分析链路、先接通分析器到扬声器的输出路径最后才接管音频元素（失败时不接管、不影响发声并自动重试），配合播放期间定时 resume 挂起上下文，彻底消除无声问题",
                "• 修复选定专辑后收起侧边栏时，左上角展开按钮与「返回专辑列表」按钮位置重叠、互相遮挡的问题：两个按钮此前都定位在主播放区左上角相近坐标，折叠侧边栏且处于专辑详情态时会同时显示并完全重合；已将折叠态下的返回按钮下移至展开按钮正下方并隐藏文字只保留圆形箭头，两个按钮互不遮挡",
                "• 修复点击折页启用自定义歌曲背景后，进入播放器设置再返回时每首歌曲的自定义背景会默认关闭、需要重新展开的问题：进入设置模式时原先不仅临时移除了折页展开样式类，还错误地把折页展开状态写回 false 并持久化保存，导致用户的展开偏好被抹除；已改为进入设置/专辑浏览仅临时隐藏视觉类、不修改持久化状态，并在每次载入曲目时按保存的展开偏好与当前曲目背景自动恢复展开，设置返回、专辑切换等场景下背景均保持启用",
                "[color:#d45d79]【感谢支持】[/color]",
                "感谢您对 PRE Launcher 的持续关注和支持！本次版本更新带来了全新重置改版的音乐播放器——从播放列表管理、本地音频导入、专辑浏览到随音乐律动的可视化播放样式，致力于为您打造功能完整、体验细腻的本地音乐播放体验；同时也欢迎体验抽卡模拟器声明弹窗与百宝箱、游戏中心的全新卡片视觉。希望能为您带来更丰富、更沉浸的使用体验！",
                "如果您在使用过程中发现任何问题，或有任何想法和建议，欢迎通过 Github 仓库提交 Issue 与我们进行沟通。",
                "[color:black]© 2014-2026 PREAlmax. All rights reserved.[/color]"
            ]
        },
